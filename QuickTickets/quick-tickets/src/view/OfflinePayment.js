...
function ShowUnpaidTickets({
  ...
  const showCaptcha = (transactionID) => {
    setCaptcha((prevState) => {
      ...
      // Pokaż captche
      if (transactionID) newCaptcha[transactionID] = !prevState[transactionID];
  ...
  return (
    <div className="searchList">
      ...
      {unpaidTickets !== undefined && unpaidTickets.totalCount > 0 ? (
        unpaidTickets.transactions?.map((transaction, index) => (
          ...
          >
            ...
            <div className="organisers-list-row">
              Data urodzenia:{" "}
              {moment(transaction.user.dateOfBirth).format(
            ...
            <div className="organisers-list-row">
              {showButtons ? (
                <React.Fragment>
                  <button
                    className="main-btn2"
                    // onClick={() => showCaptcha(transaction.transactionID)}
                    onClick={() => payOffline(transaction.transactionID)}
                  >
                    Opłać
                  </button>
                  <button
                    ...
                    onClick={() =>
                      cancelUnpaidTicket(transaction.transactionID)
                    }
                  >
                    Anuluj
                  </button>
            ...
            {/* {captcha[transaction.transactionID] && (
              <ReCAPTCHA
                sitekey="6LdYLaAqAAAAAFF3-__IpRrI2TrdeCbJ_bqWf1D9"
                onChange={(value) =>
                  payOffline(transaction.transactionID, value)
                }
              />
            )} */}
          </div>
      ...
      {unpaidTickets !== undefined && unpaidTickets.totalPages > 0 && (
        <Pagination
          count={unpaidTickets?.totalPages}
          size="large"
          onChange={(e, v) => {
            setpageIndex(v);
            showCaptcha();
        ...
        <Box sx={{ width: "100%", backgroundColor: "white" }}>
          ...
          <TabPanel value={value} index={0}>
            <TransactionController>
              <ShowUnpaidTickets showButtons={true} />
            </TransactionController>
          </TabPanel>
          <TabPanel value={value} index={1}>
            <TransactionController>
              <ShowUnpaidTickets showAll={true} />
            </TransactionController>
          </TabPanel>
        </Box>
      </main>
